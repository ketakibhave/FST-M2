...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "ketu") // Pass request body from file
        ...
        response.then().body("username", equalTo("ketu"));
    ...
    public void deleteUser() throws IOException {
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "ketu") // Add path parameter
                        .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("ketu"));
    }
}